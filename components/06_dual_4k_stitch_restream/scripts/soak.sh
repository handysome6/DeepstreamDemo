...
echo
echo "--- soak summary ---"
grep -E "p06 status|stream LIVE|stream STALLED|pipeline error|pair DEGRADED|pair RECOVERED|restream READY" "$LOG" | tail -60 || true
echo
...
PUSHED_COUNT=$(grep -c "restream READY" "$LOG" || true)
...
echo "restream startup messages:   $PUSHED_COUNT"
echo "log: $LOG"

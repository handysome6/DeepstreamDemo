...
bool RtspInferSource::start() {
    ...
    qInfo().noquote() << QString(
        "Starting RTSP+infer pipeline: uri=%1 latency=%2ms dropOnLatency=%3 "
        ...
        .arg(m_opts.muxWidth)
        .arg(m_opts.muxHeight)
        .arg(m_opts.inferConfigPath);
...
bool RtspInferSource::buildInferPipeline() {
    if (m_opts.inferConfigPath.isEmpty() ||
        ...
        qWarning().noquote() << QString(
            "Inference config not found: '%1'. Pass --infer-config or run "
            "scripts/fetch_models.sh.").arg(m_opts.inferConfigPath);
        return false;
    }
...
void RtspInferSource::linkUriPadToNext(GstPad* new_pad) {
    ...
    GstPad* sinkPad = gst_element_request_pad_simple(m_streammux, "sink_0");
    if (!sinkPad) {
    ...
    if (GST_PAD_LINK_FAILED(ret)) {
        qWarning() << "Failed to link nvurisrcbin src pad to nvstreammux sink_0:" << ret;
        emit pipelineError(QStringLiteral("nvurisrcbin -> nvstreammux link failed"));
    } else {
        qInfo() << "Linked nvurisrcbin src pad to nvstreammux sink_0.";
    }
}

...
StitchGLWidget::StitchGLWidget(QWidget* parent) : QOpenGLWidget(parent) {}
...
void StitchGLWidget::setStatusLine(QString text) {
    m_statusLine = std::move(text);
}

void StitchGLWidget::releaseCudaInterop() {
    if (m_cudaGlResource) {
...
bool StitchGLWidget::ensureTextureForFrame(const StitchedFrame& holder) {
...
bool StitchGLWidget::uploadFrameToTexture(const StitchedFrame& holder) {
...
void StitchGLWidget::paintGL() {
    ...
    if (m_statsTimer.elapsed() >= 1000) {
        ...
        qInfo().noquote() << QString(
            "fps=%1 stitch-to-paint avg=%2 ms max=%3 ms | tex=%4x%5 | %6")
            .arg(m_frameCount)
            .arg(avg, 0, 'f', 1)
            .arg(m_latencyMaxMs)
            .arg(m_textureWidth)
...
void StitchGLWidget::onNewFrame(StitchedFrame* holder) {
    if (m_nextHolder) {
        delete m_nextHolder;
    }
    m_nextHolder = holder;
